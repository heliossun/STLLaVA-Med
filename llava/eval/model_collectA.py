...
def get_answer(qs,tokenizer, model, image_tensor):
...
def eval_model(args):
    ...
    questions = questions = json.load(open(os.path.expanduser(args.question_file), "r"))
    ...
    for line in tqdm(questions):
        ...
        for qs in line["prompt"]:
            ...
            set_seed(40)
            a1 = get_answer(qs,tokenizer, model, image_tensor)
            set_seed(42)
            a2 = get_answer(qs,tokenizer, model, image_tensor)
            ans_file.write(json.dumps({"img": image_file,
                                    "prompt": cur_prompt,
                                    "answers": [a1,a2],
                                    }) + "\n")
            ans_file.flush()
