            ...
            if has_image and sentence["from"] == "human":
                ...
                if j > 0 and random() < sq_r:
                    ...
                    conv.append_message(conv.roles[2],
                                        sentence["value"] + conv.sep2)  # user --> vsuer: train to ask question
                else:
                    conv.append_message(role, sentence["value"])
                # else:
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            chosen_sources = copy.deepcopy([e["chosen_conversations"] for e in sources])
            reject_sources = copy.deepcopy([e["reject_conversations"] for e in sources])
...
def setup_model(model_args, data_args, training_args):
    ...
    if training_args.lora_enable:
        ...
        lora_config = LoraConfig(
            ...
            target_modules=find_all_linear_names(model,
                                                 ['mm_projector', 'vision_tower', 'vision_resampler', 'latent_tokens',
                                                  'crossA_layer']),
    ...
    if model_args.vision_tower is not None:
        ...
        rank0_print("v-lora param:", sum(p.numel() for p in vision_tower.parameters() if p.requires_grad))
    if training_args.bits in [4, 8]:
...
def train():
    ...
    # setup llava preference model
    model, tokenizer,vision_tower = setup_model(model_args, data_args, training_args)
    ...
    if training_args.lora_enable:
        ...
        reverse_lora=True
    if training_args.vit_lora_enable:
        training_args.vit_lora_enable = False
        reverse_vlora=True
    llava_ref_model, _, _ = setup_model(model_args, data_args, training_args)
    if reverse_lora:
        training_args.lora_enable = True
    if reverse_vlora:
        training_args.vit_lora_enable = True
    #llava_ref_model=model
